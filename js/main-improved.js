...
const SimuladorModule = (function() {
  ...
  function calcularTotales() {
    ...
    if (!dniClienteInput.value.trim()) {
      mostrarNotificacion("Por favor, ingrese el DNI del cliente", "error");
      dniClienteInput.focus();
      return;
    }
    
    // Validar formato del DNI
    if (!validarDNI(dniClienteInput.value.trim())) {
      mostrarNotificacion("El DNI ingresado no es válido. Debe tener 8 números y una letra", "error");
      dniClienteInput.focus();
      return;
    }
    ...
    resultadoCalculado = {
      ...
      dniCliente: dniClienteInput.value.trim().toUpperCase(),
  ...
  function reAnalizar() {
    ...
    confirmarAccion("¿Está seguro de querer reiniciar el análisis? Se perderán los datos actuales.", () => {
      // Limpiar nombre del deudor y DNI
      nombreDeudorInput.value = "";
      dniClienteInput.value = "";
    ...
    GoogleSheetsModule.obtenerHistorial()
      .then(data => {
        ...
        if (!data.historial || data.historial.length === 0) {
          ...
          celda.colSpan = 10; // Actualizado para incluir DNI
          ...
          data.historial.forEach(item => {
            ...
            celdaDNI.textContent = item["DNI Cliente"] || "";
    ...
    GoogleSheetsModule.obtenerDetallesContrato(folio)
      .then(data => {
        ...
        const contrato = {
          ...
          dniCliente: data.contrato["DNI Cliente"] || "",
...
const PlanLiquidacionModule = (function() {
  ...
  function mostrarPlan(datos) {
    ...
    document.getElementById("plan-dni-cliente").textContent = datos.dniCliente;
